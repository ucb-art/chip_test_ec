...
class TracePlotBase(object, metaclass=abc.ABCMeta):
    ...
    def _send_focus(self, t_idx, y_idx, cur_idx):
        t0 = self.tvec[t_idx]
        for toff in self.toff_list[cur_idx:]:
            self.thread.send(dict(tval=t0 + toff, y_idx_list=[y_idx], val=2))

    def _send_mark(self, t_idx, y_idx_list, cur_idx, otype):
        t0 = self.tvec[t_idx]
        toff = self.toff_list[cur_idx]
        self.thread.send(dict(tval=t0 + toff, y_idx_list=y_idx_list, val=otype))

    def eval_y(self, t_idx, y_idx, cur_idx, bot_edge_intv, top_edge_intv):
        self.set_offset(self.yvec[y_idx])
        self._send_focus(t_idx, y_idx, cur_idx)
        if self.thread.stop:
            raise StopException()

        output_types = self.read_trace()
        for cur_idx, (bintv, tintv, otype) in enumerate(islice(zip(bot_edge_intv, top_edge_intv,
                                                                   output_types), cur_idx, None)):
            if otype < 0:
                bintv[0] = max(bintv[0], y_idx)
                tintv[0] = max(tintv[0], y_idx)
            elif otype == 0:
                bintv[1] = min(bintv[1], y_idx)
                tintv[0] = max(tintv[0], y_idx)
            else:
                bintv[1] = min(bintv[1], y_idx)
                tintv[1] = min(tintv[1], y_idx)
    ...
    def find_edge(self, is_bot, t_idx, cur_idx, bot_edge_intv, top_edge_intv, guess_idx):
        ...
        self.eval_y(t_idx, guess_idx, cur_idx, bot_edge_intv, top_edge_intv)
        # binary search to find edge of current index
        while cur_intv[1] > cur_intv[0] + 1:
            y_idx = (cur_intv[1] + cur_intv[0]) // 2
            self.eval_y(t_idx, y_idx, cur_idx, bot_edge_intv, top_edge_intv)
        ...
        try:
            for t_idx, tval in enumerate(self.tvec):
                ...
                for cur_idx in range(num_idx):
                    ...
                    top_edge = min(num_y - 1, top_edge_intv[cur_idx][0] + 1)
                    self._send_mark(t_idx, list(range(0, bot_edge + 1)), cur_idx, -1)
                    self._send_mark(t_idx, list(range(bot_edge + 1, top_edge)), cur_idx, 0)
                    self._send_mark(t_idx, list(range(top_edge, num_y)), cur_idx, 1)
...
class TracePlotFake(TracePlotBase):
    ...
    def __init__(self, thread: WorkerThread, ctrl: Controller, config: Dict[str, Any]):
        ...
        tper = int(round(1e12 / config['data_rate']))
        self.output_len = config['output_len']
        self.ym = (y1 + y0) / 2
        self.yper = t1 - t0 + self.output_len * tper
        self.tr_w = (y1 - y0) / 20
        self.amp = (y1 - y0) / 4
        self.t_cur = t0
        self.y_cur = y0
